...
public final class SSLEngineImpl extends SSLEngine
{
    ...
    private static final Logger logger = Logger.getLogger(SSLEngineImpl.class.getName());
    ...
    SSLEngineImpl(SSLContextImpl contextImpl, String host, int port)
    {
        ...
        session.suite = new CipherSuite.TLS_NULL_WITH_NULL_NULL();
        ...
        if (Debug.DEBUG)
            logger.log(Level.FINE, "generated session ID {0} with random {1}",
    ...
    {
        return new String[]{
                CipherSuite.TLS_DHE_DSS_WITH_AES_256_CBC_SHA.class.getSimpleName(),
                CipherSuite.TLS_DHE_RSA_WITH_AES_256_CBC_SHA.class.getSimpleName(),
                CipherSuite.TLS_DH_DSS_WITH_AES_256_CBC_SHA.class.getSimpleName(),
                CipherSuite.TLS_DH_RSA_WITH_AES_256_CBC_SHA.class.getSimpleName(),
                CipherSuite.TLS_RSA_WITH_AES_256_CBC_SHA.class.getSimpleName(),
                CipherSuite.TLS_DHE_DSS_WITH_AES_128_CBC_SHA.class.getSimpleName(),
                CipherSuite.TLS_DHE_RSA_WITH_AES_128_CBC_SHA.class.getSimpleName(),
                CipherSuite.TLS_DH_DSS_WITH_AES_128_CBC_SHA.class.getSimpleName(),
                CipherSuite.TLS_DH_RSA_WITH_AES_128_CBC_SHA.class.getSimpleName(),
                CipherSuite.TLS_RSA_WITH_AES_128_CBC_SHA.class.getSimpleName(),
                CipherSuite.TLS_RSA_WITH_NULL_MD5.class.getSimpleName(),
                CipherSuite.TLS_RSA_WITH_NULL_SHA.class.getSimpleName()
        };
    }
    ...
    throws SSLException
  {
    ...
    {
        if (Debug.DEBUG)
            logger.log(Level.FINE, "{0} handshake begins", mode);
    ...
    public
    @Override
    void setWantClientAuth(final boolean wantClientAuth)
    {
    ...
    public
    @Override
    SSLEngineResult unwrap(final ByteBuffer source,
            ...
            throws SSLException
    {
        ...
        if (Debug.DEBUG)
            logger.log(Level.FINE, "input record:\n{0}", record);
        ...
        ByteArrayOutputStream sysMsg = null;
        ByteBuffer msg = null;
        ...
        try
        {
            ...
            {
                if (insec.cipherSuite().getClass().equals(CipherSuite.TLS_NULL_WITH_NULL_NULL.class))
                    msg = record.fragment();
                else
                {
                    sysMsg = new ByteArrayOutputStream();
        ...
        catch (BufferOverflowException boe)
        {
            ...
            logger.log(Level.INFO, "buffer overflow when decrypting", boe);
            return new SSLEngineResult(SSLEngineResult.Status.BUFFER_OVERFLOW,
                    handshakeStatus, 0, 0);
        }
        catch (IllegalBlockSizeException|MacException e)
        {
            lastAlert = new Alert(Alert.Level.FATAL,
                    Alert.Description.BAD_RECORD_MAC);
            throw new AlertException(lastAlert, e);
        }
        ...
        catch (ShortBufferException|IOException e)
        {
        ...
        {
            if (Debug.DEBUG)
                logger.log(Level.FINE, "sysmessage {0}", sysMsg);
            msg = ByteBuffer.wrap(sysMsg.toByteArray());
        }
        ...
        else if (type == ContentType.ALERT)
        {
            ...
            if (Debug.DEBUG)
                logger.log(Level.FINE, "processing alerts {0}",
                           Util.wrapBuffer(msg));
            len += msg.remaining() / 2;
            ...
            if (Debug.DEBUG)
                logger.log(Level.FINE, "alerts: {0}", alerts.length);
        ...
        else if (type == ContentType.HANDSHAKE)
        {
            ...
            if (Debug.DEBUG)
                logger.log(Level.FINE, "handshake status {0}", handshakeStatus);
            result = new SSLEngineResult(SSLEngineResult.Status.OK,
                    handshakeStatus,
                    record.length() + 5,
                    0);
        ...
        if (Debug.DEBUG)
            logger.log(Level.FINE, "return result: {0}", result);
    ...
    public
    @Override
    SSLEngineResult wrap(ByteBuffer[] sources, int offset, int length,
                         ByteBuffer sink)
            throws SSLException
    {
        if (mode == null)
        ...
        if (Debug.DEBUG)
            logger.log(Level.FINE, "wrap {0} {1} {2} {3} / {4}",
                       new Object[] { sources, offset, length, sink, getHandshakeStatus() });
        if (lastAlert != null)
        {
        ...
        else if (getHandshakeStatus() == SSLEngineResult.HandshakeStatus.NEED_WRAP)
        {
            ...
            // the buffer directly.
            if (outsec.suite().getClass().equals(CipherSuite.TLS_NULL_WITH_NULL_NULL.class))
            {
                ...
                if (Debug.DEBUG)
                    logger.log(Level.FINE, "emitting record:\n{0}",
            ...
            sysMessage.flip();
            if (Debug.DEBUG)
                logger.log(Level.FINE, "handshake status {0}",
                        handshakeStatus);
        }
        ...
        try
        {
            ...
            {
                if (Debug.DEBUG)
                    logger.log(Level.FINE, "encrypt system message {0} to {1}", new Object[] { sysMessage, sink });
        ...
        catch (ShortBufferException|IllegalBlockSizeException|DataFormatException e)
        {
