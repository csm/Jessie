...
public class SSLSocketImpl extends SSLSocket
{
    ...
    private static final Logger logger = Logger.getLogger(SSLSocketImpl.class.getName());
    ...
    void doHandshake() throws IOException
    {
        ...
        try
        {
            ...
            {
                logger.log(Level.FINE, "socket processing state {0}",
                           status);
            ...
            if (Debug.DEBUG)
                logger.log(Level.FINE,
                        "handshake completed in {0}ms in thread {1}",
                        new Object[] { now, Thread.currentThread().getName() });
        }
