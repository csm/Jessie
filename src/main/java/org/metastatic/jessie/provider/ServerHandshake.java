...
class ServerHandshake extends AbstractHandshake
{
    ...
    private static CompressionMethod chooseCompression(final CompressionMethodList comps)
            throws SSLException
    {
        String enable = AccessController.doPrivileged((PrivilegedAction<String>) () -> Security.getProperty("org.metastatic.jessie.enable.compression"));
        // Scan for ZLIB first.
    ...
    public
    @Override
    HandshakeStatus implHandleInput()
            throws SSLException
    {
        if (state == DONE)
        ...
        if (Debug.DEBUG)
            logger.log(Level.FINE, "processing in state {0}:\n{1}",
                       new Object[] { state, handshake });
        ...
        switch (state)
        {
...

            {
                ...
                if (Debug.DEBUG)
                    logger.log(Level.FINE,
                            ...
                            new Object[] { engine.session().version, engine.session().suite,
                            compression });
                ...
                if (Debug.DEBUG)
                    logger.log(Level.FINE, "looked up saved session {0}", s);
            ...
            case READ_CLIENT_KEY_EXCHANGE:
            {
                ...
                switch (alg)
                {
                    ...
                    case DH_anon:
                    {
                        ...
                        DHPublicKey clientKey = new DHPublicKeyImpl(pub.publicValue(), myKey.getParams());
                    ...
                    case DHE_PSK:
                    {
                        ...
                        DHPublicKey clientKey = new DHPublicKeyImpl(params.params().publicValue(), serverKey.getParams());
                        SecretKey psk = null;
                        try
                        {
            ...
            case READ_FINISHED:
            {
                ...
                if (engine.session().version == ProtocolVersion.TLS_1_2)
                {
                    try
                    {
                        digests = new MessageDigest[] { (MessageDigest) sha256.clone() };
                    }
                ...
                try
                {
                    serverFinished = new Finished(generateFinished(digests, true, engine.session()),
                            engine.session().version
                    );
                }
                catch (InvalidAlgorithmParameterException | NoSuchAlgorithmException e)
                ...
                if (Debug.DEBUG)
                    logger.log(Level.FINE, "server finished: {0}",
                               serverFinished);

                if (!MessageDigest.isEqual(clientFinished.verifyData(),
    ...
    public
    @Override
    HandshakeStatus implHandleOutput(ByteBuffer fragment)
            throws SSLException
    {
        if (Debug.DEBUG)
            logger.log(Level.FINE,
                       "handle output state: {0}; output fragment: {1}",
                       new Object[] { state, fragment });
        ...
        while (fragment.remaining() >= 4 && state.isWriteState())
        {
            switch (state)
            {
                ...
                case WRITE_HELLO_REQUEST:
                {
                    ...
                    if (Debug.DEBUG)
                        logger.log(Level.FINE, "{0}", handshake);
                    state = READ_CLIENT_HELLO;
                }
                ...
                case WRITE_SERVER_HELLO:
                {
                    ...
                    if (continuedSession)
                    {
                        TLSSessionKeys keys = null;
                        try
                        {
                            keys = generateKeys(clientRandom, serverRandom,
                                    engine.session());
                        }
                    ...
                    else if (kex == DHE_DSS || kex == DHE_RSA || kex == RSA || kex == RSA_PSK)
                    {
                ...
                case WRITE_CERTIFICATE:
                {
                    ...
                    if (Debug.DEBUG)
                    {
                        logger.log(Level.FINE, "my cert:\n{0}", localCert);
                        logger.log(Level.FINE, "{0}", cert);
                    }
                ...
                case WRITE_SERVER_HELLO_DONE:
                {
                    ...
                    if (Debug.DEBUG)
                        logger.log(Level.FINE, "writing ServerHelloDone");
                ...
                case WRITE_FINISHED:
                {
                    ...
                    try
                    {
                        outBuffer = generateFinished(digests, false, engine.session());
                    }
                    catch (InvalidAlgorithmParameterException | NoSuchAlgorithmException e)
                    {
                        throw new SSLException(e);
            ...
            throws NoSuchAlgorithmException, InvalidKeyException, SignatureException
    {
        ...
        if (Debug.DEBUG_KEY_EXCHANGE)
            logger.log(Level.FINE, "server key: {0}", key);
        sig.initSign(key);
    ...
    private class GenDH extends DelegatedTask
    {
                ...
                InvalidKeyException, SignatureException
        {
            ...
            DHParameterSpec dhparams = DiffieHellman.getParams();
